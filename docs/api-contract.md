...
| `priceHistory(range): PriceSeries` | NADAC snapshot | **snapshot (weekly)** | **Nullable, and currently always null — there is no resolver.** It was `PriceSeries!`, which did not degrade: the non-null propagated up and `drug` itself came back null, so selecting one unimplemented field killed the whole page (measured 2026-09-16: `Cannot return null for non-nullable field Drug.priceHistory`). Null means the series could not be built, which today is always — the snapshot holds one current price per NDC (~3 MB) and a series needs full history (~102 MB), whose retention shape ADR-009 flags as a decision the sync job will force. **Knowingly weaker than the rest of the schema:** a bare null cannot say whether history is unpublished or merely unstored, where ADR-010 would state the absence. The ADR-010-shaped answer is a `PriceSeriesResult` union; null is the honest interim. When it resolves, the series always does so with `coverage`, and the *points* may be empty. |
