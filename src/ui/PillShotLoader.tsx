...
/* ------------------------------------------------------------------
   ...
   Colours live in ./PillShotLoader.palette — see that file for why they
...
type TextPop = { x: number; y: number; text: string; life: number };

type Aim = { sx: number; sy: number; cx: number; cy: number };
...
type Game = {
  ...
  particles: Particle[];
  texts: TextPop[];
  loaded: { pal: Palette; appear: number } | null;
  next: Palette;
  reload: number;
  aim: Aim | null;
  kb: { active: boolean; angle: number; power: number };
  shots: number;
  score: number;
  streak: number;
  amp: number;
...
type BottleGeom = { bx: number; by: number; bw: number; bh: number };

type LidSeg = { a: Vec; b: Vec; thick: number };

type Pull = { px: number; py: number; len: number };

/* ---------------------------- helpers ---------------------------- */

const clamp = (v: number, a: number, b: number) => Math.max(a, Math.min(b, v));

const randPal = (): Palette =>
  PALETTES[Math.floor(Math.random() * PALETTES.length)] ?? DEFAULT_PALETTE;
...
const SHELVES: { lines: number[]; items: ShelfItem[] } = (() => {
  ...
  for (const y of lines) {
    ...
    while (x < W - 50) {
      const w = 26 + rnd() * 40;
      const h = 40 + rnd() * 80;
      const c = SHELF_COLORS[Math.floor(rnd() * SHELF_COLORS.length)] ?? INK;
      items.push({ x, y, w, h, c });
      x += w + 8 + rnd() * 22;
    }
  }
  return { lines, items };
...
function bottleGeom(cx: number): BottleGeom {
  return {
    bx: cx - BOTTLE_W / 2,
    by: GROUND - BOTTLE_H,
    bw: BOTTLE_W,
...
function lidAngle(g: Game) {
  return LID_OPEN + g.lidKick * Math.sin(g.t * 18);
}

function lidSegment(b: BottleGeom, angle: number): LidSeg {
  const px = b.bx + b.bw + 2;
  const py = b.by - 4;
  const c = Math.cos(angle);
  const s = Math.sin(angle);
  const tf = (lx: number, ly: number): Vec => ({
    x: px + lx * c - ly * s,
    y: py + lx * s + ly * c,
  });
  return {
    a: tf(-(CAP_W - 12), -CAP_H / 2),
    b: tf(-8, -CAP_H / 2),
    thick: CAP_H / 2 - 2,
  };
}

const pulledPos = (px: number, py: number): Vec => ({
  x: LAUNCH.x - px * 0.28,
  y: LAUNCH.y - py * 0.28,
});

function currentPull(g: Game): Pull | null {
  let px: number;
  let py: number;
  if (g.aim) {
...
function bounce(p: Pill, nx: number, ny: number, rest: number) {
  ...
  if (vn < 0) {
    ...
    p.vx *= 0.92;
    p.vy *= 0.92;
    if (Math.abs(p.spin) < 2)
      p.spin = (p.vx >= 0 ? 1 : -1) * (5 + Math.random() * 5);
  }
}

type Rect = { x: number; y: number; w: number; h: number };

function collideRect(p: Pill, r: Rect, rest: number) {
  ...
  if (d < 1e-4) {
    // Deepest overlap wins. Picked by scan rather than sort+index so the
    // result is a value, not a possibly-undefined element.
    let best: readonly [number, number, number] = [p.x - r.x, -1, 0];
    const candidates: ReadonlyArray<readonly [number, number, number]> = [
      [r.x + r.w - p.x, 1, 0],
      [p.y - r.y, 0, -1],
      [r.y + r.h - p.y, 0, 1],
    ];
    for (const cand of candidates) if (cand[0] < best[0]) best = cand;
    nx = best[1];
    ny = best[2];
    p.x += nx * (best[0] + PILL_R);
    p.y += ny * (best[0] + PILL_R);
  } else {
    nx = dx / d;
    ny = dy / d;
    p.x += nx * (PILL_R - d);
...
function collideSeg(p: Pill, seg: LidSeg, rest: number) {
  const abx = seg.b.x - seg.a.x;
  const aby = seg.b.y - seg.a.y;
  const t = clamp(
    ((p.x - seg.a.x) * abx + (p.y - seg.a.y) * aby) / (abx * abx + aby * aby),
    0,
    1,
  );
...
function scorePill(g: Game, p: Pill, b: BottleGeom, reduced: boolean) {
  ...
  if (!reduced) {
    g.lidKick = 0.25;
    g.squash = 1;
    burst(g, b.bx + b.bw / 2, b.by, [p.pal.a, p.pal.b, SPARK], 14);
  }
...
function update(g: Game, dt: number, reduced: boolean) {
  g.t += dt;
  ...
  if (g.stackClearAt && g.t >= g.stackClearAt) {
    ...
    g.stackClearAt = 0;
    if (!reduced)
      burst(g, b.bx + b.bw / 2, b.by, FULL_BOTTLE_SPARKS, 26);
  }
...
function capsulePath(ctx: Ctx, len: number, w: number) {
  const r = w / 2;
  const half = len / 2;
  ctx.beginPath();
  ctx.moveTo(-half + r, -r);
...
function withSquash(ctx: Ctx, g: Game, b: BottleGeom, fn: () => void) {
...
function drawLid(ctx: Ctx, b: BottleGeom, angle: number) {
  ctx.save();
  ctx.translate(b.bx + b.bw + 2, b.by - 4);
  ctx.rotate(angle);
  const x0 = -(CAP_W - 4);
  rrect(ctx, x0, -CAP_H, CAP_W, CAP_H, 9);
  ctx.fillStyle = BOTTLE.capBody;
  ctx.fill();
  ctx.fillStyle = BOTTLE.capHighlight;
  rrect(ctx, x0, -CAP_H, CAP_W, 11, 9, 2);
  ctx.fill();
  ctx.strokeStyle = BOTTLE.capRidge;
  ctx.lineWidth = 2;
  ctx.lineCap = "round";
...
function draw(ctx: Ctx, g: Game, font: string, showShelves: boolean) {
  ...
  withSquash(ctx, g, b, () => {
    ...
    ctx.fillStyle = BOTTLE.back;
    ctx.fill();
    ctx.save();
    rrect(ctx, b.bx, b.by, b.bw, b.bh, 6, 30);
    ctx.clip();
    for (const e of g.stack)
      drawPill(ctx, b.bx + e.ox, GROUND + e.oy, e.angle, e.pal, 0.72);
    ctx.restore();
  });
  ...
  withSquash(ctx, g, b, () => {
    ...
    ctx.fillStyle = BOTTLE.rim;
    ctx.fill();
    ctx.fillStyle = BOTTLE.rimShadow;
...
export type PillShotLoaderProps = {
  /** While true the status chip spins; false swaps it for the ready card. */
  isLoading?: boolean;
  message?: string;
  readyMessage?: string;
  ...
  onContinue?: () => void;
  onSkip?: () => void;
  /** URL for a background photo. Without one, faint shelves are drawn. */
...
export function PillShotLoader({
  ...
  const bgRef = useRef<string | undefined>(backgroundImage);

  // The render loop reads the current background without being torn down and
  // rebuilt when it changes. Writing the ref during render would be a
  // render-phase side effect (react-hooks/refs), so it is synced in an effect.
  useEffect(() => {
  ...
  const toWorld = (e: PointerEvent<HTMLCanvasElement>): Vec | null => {
    const canvas = canvasRef.current;
  ...
  return (
    ...
    >
      ...
      {/* Status */}
      <div
        style={{ position: "absolute", top: "3%", left: "2.5%" }}
        aria-live="polite"
      >
        <div style={chip}>
          {ready ? (
          ...
          <span className={ready ? undefined : "psl-dots"}>
            {ready ? readyMessage : message}
          </span>
        </div>
      </div>
      ...
      >
        <div style={chip}>
          ...
          {streak >= 2 && (
            <span style={{ color: accentColor }}>{streak} in a row</span>
          )}
        </div>
      </div>
      ...
      {ready && !dismissed && (
        ...
        >
          ...
          >
            Your prices are ready
          </div>
          <div style={{ opacity: 0.65, marginBottom: "0.9em" }}>
            {score > 0
              ? `You scored ${score}. Nice shooting.`
              : "We found the lowest prices we could."}
          </div>
          <div
            style={{ display: "flex", gap: "0.6em", justifyContent: "center" }}
          >
            {onContinue && (
              <button
                className="psl-btn"
