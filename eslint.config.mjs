...
const eslintConfig = defineConfig([
...

  {
    ...
    //
    // The loading mini games are canvas artwork, not a design system: a warm
    // candy palette with no overlap with the product's five cool tokens and no
    // semantic role. They are also unreachable by the token layer in principle
    // — a canvas `fillStyle` takes a colour string and cannot resolve
    // `var(--x)`.
    //
    // One palette module, shared by every game, so adding a game does not widen
    // the hole. The components that consume it stay under the rule, so a stray
    // hex in game logic still fails the build.
    files: ["src/ui/game.palette.ts"],
